from gcsl.algo import buffer, networks_n
...
def get_params(env, env_params):
    ...
    policy = default_markov_policy_n(env, env_params)
    buffer_kwargs = dict(
        env=env,
...
def default_markov_policy_n(env, env_params):
    assert isinstance(env.action_space, Discrete)
    if env.action_space.n > 100: # Too large to maintain single action for each
        policy_class = networks_n.IndependentDiscretizedStochasticGoalPolicy
    else:
        policy_class = networks_n.DiscreteStochasticGoalPolicy_n
    return policy_class(
                env,
